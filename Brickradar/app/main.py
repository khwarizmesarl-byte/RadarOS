...
def _lsf() -> str:
    """Latest snapshot filter — local stores only (for analytics/advanced)."""
    return "AND source_type='local' AND captured_at IN (SELECT MAX(captured_at) FROM snapshots GROUP BY store)"
