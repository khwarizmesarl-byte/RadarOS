...
ADMIN_PIN = os.getenv("ADMIN_PIN", "1234")  # Change in .env to secure
...
# Ayoub Computers and others added by user via Store Manager
BIGCOMMERCE_STORES = {
    "Ayoub Computers": {
...
def extract_item_number(text: str) -> Optional[str]:
    ...
    # Round numbers that appear in product names/descriptions but are NOT set numbers
    _ROUND = {"1000","2000","3000","4000","5000","1500","2500",
              "1100","1200","1300","1400","1600","1700","1800","1900"}
    _QTY = re.compile(r"^(piece|pieces|pcs|pc|parts|bricks|blocks|elements|in1)")

    # Explicit Item # tag — most reliable
    m = re.search(r"Item\s*#\s*([0-9]{4,7})", t, flags=re.IGNORECASE)
    if m:
        return m.group(1)

    # SKU like "10345-LEGO" — skip if round number (e.g. "1000-RR" is a model name, not a set number)
    m = re.search(r"\b([0-9]{4,7})-[A-Za-z]+\b", t)
    if m and m.group(1) not in _ROUND:
        return m.group(1)

    # Collect all candidates, filtering out quantity descriptors
    candidates = []
    for m in re.finditer(r"\b([0-9]{4,7})\b", t):
        num = m.group(1)
        after = t[m.end():m.end()+10].strip().lower()
        if _QTY.match(after):
            continue  # skip — this is a quantity not a set number
        candidates.append((m.start(), num))

    if not candidates:
        return None

    # Prefer non-round numbers — actual set numbers are never round thousands
    non_round = [num for _, num in candidates if num not in _ROUND]
    if non_round:
        return non_round[-1]  # last non-round = most likely the set number
...
def db_init() -> None:
    ...
    """)
    # Add unique constraint migration for existing DBs
    try:
    ...
    with httpx.Client(timeout=DEFAULT_TIMEOUT, follow_redirects=True, headers=HEADERS) as client:
        ...
        while True:
            ...
            for prod in products:
                ...
                # Prefix item_number with brand for non-LEGO to avoid collisions
...
def refresh_all() -> Dict[str, Any]:
    ...
    # Now run the rate-sensitive stores sequentially
    for t in late_tasks:
        try:
            store_name, cat = _run(t)
            results[store_name] = cat
        except Exception as e:
            print(f"[refresh_all] late fetch error: {e}")
...
def api_refresh_stream(stores: str = ""):
    ...
    Optional ?stores=Store1,Store2 to refresh only specific stores.
    """
    ...
    def event_stream():
        ...
        if selected:
            def _task_label(t):
                return t[1] or ("BRICKSHOP" if t[0] == "brickshop" else ("PlayOne" if t[0] == "playone" else t[0]))
            tasks = [t for t in tasks if _task_label(t) in selected]
        ...
        # Brix & Figures and Thetoystorelb are scraped SEQUENTIALLY after all others
        # to avoid 429s — they share hosting that blocks concurrent requests
        SEQUENTIAL_AFTER = {"Brix & Figures", "Thetoystorelb"}
        main_tasks = [t for t in tasks if (t[1] or t[0]) not in SEQUENTIAL_AFTER]
        late_tasks  = [t for t in tasks if (t[1] or t[0]) in SEQUENTIAL_AFTER]
        ...
        def _run_late_sequential():
            # Wait for all main tasks to finish before starting these
            time.sleep(shopify_idx * 5 + 10)
            for lt in late_tasks:
                _run_and_report(lt, 0)
                time.sleep(10)

        if late_tasks:
            pool.submit(_run_late_sequential)
...
def api_get_stores():
    ...
    for bc_name, bc_cfg in BIGCOMMERCE_STORES.items():
        hardcoded.append({"id": None, "name": bc_name, "base_url": bc_cfg["base_url"], "platform": "bigcommerce", "vat_multiplier": bc_cfg.get("vat_multiplier", 1.0), "collection_slug": bc_cfg.get("category_slug"), "new_arrivals_collection": None, "enabled": 1, "hardcoded": True, "lego_only": bc_cfg.get("lego_only", True)})
...
def api_get_settings():
    return JSONResponse({
        ...
        "groq_key_preview": ("*" * 8 + GROQ_API_KEY[-4:]) if len(GROQ_API_KEY) > 4 else ("set" if GROQ_API_KEY else ""),
    })

@app.post("/api/settings/save")
async def api_save_settings(request: Request):
    global GROQ_API_KEY, ANTHROPIC_API_KEY
    body = await request.json()
    ...
    return JSONResponse({"ok": True, "message": "Settings saved. AI assistant is now active."})
...
@app.post("/api/db/clear-snapshots")
async def db_clear_snapshots(request: Request):
    body = await request.json()
    store = body.get("store")  # None = all stores
    conn = db_connect()
    cur = conn.cursor()
    try:
        if store:
            cur.execute("DELETE FROM snapshots WHERE store=?", (store,))
        else:
...
def db_stats():
    import os as _os
    db_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "lego_tracker.db")
    try:
    ...
    try:
        ...
        cur.execute("""
            SELECT store, COUNT(*) as rows, MAX(captured_at) as last_refresh
            FROM snapshots GROUP BY store ORDER BY rows DESC
        """)
...
def api_fetch_all_logos():
    ...
    hardcoded = [
        ("BRICKSHOP",       "https://brickshop.me"),
        ("PlayOne",         "https://playone.com.lb"),
    ] + [(name, cfg["base_url"]) for name, cfg in BIGCOMMERCE_STORES.items()]
...
def build_context(page: str = "dashboard") -> str:
    ...
    if page == "analytics":
        cur.execute(f"SELECT tags, COUNT(*) as cnt FROM snapshots WHERE tags IS NOT NULL AND tags != \'\' {lsf} GROUP BY tags ORDER BY cnt DESC LIMIT 15")
...
Be concise. Format prices as $XX.XX."""
        ...
        cur.execute(f"""
            SELECT item_number, title, COUNT(DISTINCT store) as sc,
                   MIN(price) as mn, MAX(price) as mx, ROUND(MAX(price)-MIN(price),2) as spread
            FROM snapshots WHERE price>0 {lsf}
            GROUP BY item_number HAVING sc >= 2 ORDER BY spread DESC LIMIT 15
        """)
        spread_lines = [f"- #{r['item_number']} {r['title']} | {r['sc']} stores | ${r['mn']}-${r['mx']} spread:${r['spread']}" for r in cur.fetchall()]
        conn.close()
        return f"""You are BrickRadar AI, a LEGO price assistant for stores in Lebanon.
Last refresh: {last_updated}
